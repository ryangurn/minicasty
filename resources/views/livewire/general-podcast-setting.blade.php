                    ...
                    <div class="px-4 py-5 bg-white sm:p-6">
                        <div class="grid grid-cols-6 gap-6">
                                ...
                                @if (isset($languages) && !$languages->isEmpty())
                                    <select wire:model="language" name="language" multiple class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('language') mb-4 @enderror ">
                                        @foreach($languages as $language)
                                            <option value="{{ $language->guid }}">{{ strtolower($language->name) }}</option>
                                        @endforeach
                                    </select>
                                @endif
                                @error('language') <span class="pt-2 pb-2 pl-4 pr-4 rounded-full text-red-700 bg-red-100">{{ strtolower($message) }}</span> @enderror
                            </div>
                                ...
                                @if (isset($c) && !$c->isEmpty())
                                    <select wire:model="categories" name="categories" multiple class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('categories') mb-4 @enderror ">
                                        @foreach($c as $category)
                                            <option value="{{ $category->guid }}">{{ strtolower($category->name) }}</option>
                                        @endforeach
                                    </select>
                                @endif
                                @error('categories') <span class="pt-2 pb-2 pl-4 pr-4 rounded-full text-red-700 bg-red-100">{{ strtolower($message) }}</span> @enderror
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-4">
                                ...
                                @error('author') <span class="pt-2 pb-2 pl-4 pr-4 rounded-full text-red-700 bg-red-100">{{ strtolower($message) }}</span> @enderror
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-4">
                                ...
                                @error('link') <span class="pt-2 pb-2 pl-4 pr-4 rounded-full text-red-700 bg-red-100">{{ strtolower($message) }}</span> @enderror
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                @error('owner_name') <span class="pt-2 pb-2 pl-4 pr-4 rounded-full text-red-700 bg-red-100">{{ strtolower($message) }}</span> @enderror
                            </div>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                @error('owner_email') <span class="pt-2 pb-2 pl-4 pr-4 rounded-full text-red-700 bg-red-100">{{ strtolower($message) }}</span> @enderror
                            </div>
